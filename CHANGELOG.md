...
- `Plans/Plan_CiliumLB_Ingress.md` — migration plan with cutover and
  rollback checklist.
- `docs/k8s-secrets.md` — catalog of every cluster Secret: source
  (SOPS-committed in `cluster/` vs runtime-generated by ECK/Helm/
  cert-manager) and exact command to read each. SOPS set is
  `grafana-admin` (ksops, monitoring-secrets Application) plus
  `argocd-admin-password`, `argocd-repo-ssh-key`, `argocd-sops-age-key`
  (applied manually at bootstrap; admin plaintext verified against the
  live bcrypt hash in `cluster/base/argocd/values.yaml`). TLS Secrets
  (`argocd-server-tls`, `monitoring-tls`, `observability-tls`,
  `longhorn-ui-tls`) issued by ClusterIssuer `step-ca`; ECK/Helm/Cilium
