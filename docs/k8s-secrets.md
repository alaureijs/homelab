...
(Cilium runtime secrets — `cilium-ca`, `hubble-server-certs` — were removed
with the Cilium→Calico migration on 2026-08-14.)
...
Certificates use HTTP-01 challenges (`solvers[0].http01.ingress` in
`cluster/base/cert-manager/issuer.yaml`) — cert-manager creates a temporary
solver Ingress with `ingressClassName: traefik` on the `.42` VIP, so the
challenge hostname must resolve to `192.168.100.42` in DNS (all service
hostnames already do via `dns_records` in `group_vars/all/main.yml`).
...
## Cilium secrets (kube-system) — REMOVED

`cilium-ca` and `hubble-server-certs` were runtime-only (no repo copy) and
were removed with the Cilium→Calico migration on 2026-08-14. No Cilium
secrets remain in `kube-system`.
