...
* **Update**: `cluster/base/cert-manager/issuer.yaml` ClusterIssuer HTTP-01
  ...
  during cutover; only `cilium` class exists). Applied live + verified:
  solver Ingress gets class `cilium` on VIP `.42`, challenge path returns
  200, issuer Ready. caBundle re-verified after apply. Test issuance
  (`cert-test.homelab.internal`) failed only on DNS (no record) — all real
  cert hostnames resolve to `.42`.
